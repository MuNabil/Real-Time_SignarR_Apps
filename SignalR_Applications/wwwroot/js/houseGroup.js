...
// [track1] Get trigger notification buttons
...
// [track1] To call the hub method to notify all users when any user click to a button
...
// [track1] Notify all house subscribers when notification triggered
connectionHouse.on("houseNotificationTriggered", (houseName) => {
  toastr.info(`${houseName} Notification triggered`);
});

// To notify all house subscribers when new user subscribe to this house
connectionHouse.on("newMemberJoinedToHouse", (houseName) => {
  toastr.success(`A new member subscribe to: ${houseName}`);
});

// To notify all house subscribers when new user unsubscribe to a particular house
connectionHouse.on("newMemberLeaveFromHouse", (houseName) => {
  toastr.warning(`Member unsubscribe from: ${houseName}`);
});

// To listen to the method that send the new list of houses after user subscribe/unsubscribe to a house
connectionHouse.on(
  "subscriptionStatus",
  (strGroupsJoined, houseName, hasSubscribed) => {
    ...
    if (hasSubscribed) {
      // when user subscribe to a house hide it from subscription list so he cannot subscribe on it again
      // and display it in the unsubscription list
      ...
      // when user unsubscribe to a house hide it from unsubscriptions list so he cannot unsubscribe to it again
      // and display it in the uubscription list
