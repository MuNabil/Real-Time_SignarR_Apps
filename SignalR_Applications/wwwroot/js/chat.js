...
connection.on("ReceiveUserConnected", (userName) => {
  addMessage(`${userName}  has openned a connection`);
});

connection.on("ReceiveUserDisconnected", function (userName) {
...
connection.on(
  ...
  function (senderId, senderName, receiverId, message, chatId, receiverName) {
    addMessage(
      `[Private Message to ${receiverName} ]${senderName} says ${message}`
    );
  }
);
...
function addnewRoom(maxRoom) {
  ...
  $.ajax({
    ...
    data: JSON.stringify({ id: 0, name: roomName }), // Arguments to send to the PostChatRoom method
    async: true,
    processData: false,
    cache: false,
...
function fillUserDropDown() {
  $.getJSON("/ChatRooms/GetChatUser")
    .done(function (json) {
      ...
      // To fill the drop down list for users
...
// When user choose a room to delete from the drop down list
function deleteRoom() {
